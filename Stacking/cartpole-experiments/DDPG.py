...
def crc(env):
        ...
        action = Input(shape=(1,actions))
        state_1=Flatten()(state)
        action_1=Flatten()(action)
        x = Dense(256, activation='relu')(state_1)
        x = concatenate([x, action_1])
        x = Dense(128, activation='relu')(x)
        out = Dense(1, activation='sigmoid')(x)
        model=Model(inputs=[state, action], outputs=out)
        plot_model(model,show_shapes=True, to_file='models/crc-ddpg.png')
...
for i in range(len(archs)):
    ...
    model_crc,ac_layer=crc(env)
    #Defining DDPG Keras-RL agent: inputing the policy and the model
    model = DDPGAgent(actor=model, critic=model_crc,critic_action_input=ac_layer  , memory=memory,nb_actions=actions)
    #Compiling DDPG with mean squared error l]oss
